...
transformed parameters {
    // parameter c 
    real<lower=0, upper=10> c = inv_logit(logit_c)*10;  // times 10 as c is bounded between 0 and 10 (deviation from the original model)
...
model {
    ...
    // Decision Data
    target += bernoulli_lpmf(y | r); // r is the probablity of chosing either category 1 or 2; this is the actual model
}
...
generated quantities {
    ...
    real<lower=0, upper=10> c_prior = inv_logit(logit_c_prior)*10; // changed from *2 to *10 because c is bounded between 0 and 10 in the r_script

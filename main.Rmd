...
# filter out category 3 and 4
data <- data %>% filter(category != 3 & category != 4)
...
# unique number of stimuli
unique(data$stimulus)
```
...
```{r}
unique(data$category==1)

head(data,10)


data_test$test <- ifelse(data$response == 3 | data$response == 4, 0, 1)
```